...
- Холст поддерживает:
  ...
  - сетку на фоне SVG.
  - контекстное меню по правому клику на пустой области (пункты "Добавить узел", "Добавить связь").
- Взаимодействие с элементами:
  - двойной клик по узлу — открывает редактор узла (текст + форма);
  - двойной клик по связи — открывает редактор связи (метка).
- Создание узла:
  - диалог `AddNodeDialog` с вводом текста и выбором формы;
  - автоматическая генерация уникального ID (`NewNode1`, `NewNode2`, …);
  - добавление в Mermaid-код с корректным синтаксисом формы;
  - автоматическое создание `layout`-хинта через `upsertLayoutHint`.
- Создание связи:
  - режим выбора: клик по исходному узлу → клик по целевому узлу;
  - после выбора открывается диалог `AddEdgeDialog` для ввода метки;
  - добавление строки связи в синтаксисе `-->` с опциональной меткой `|label|`.
- Доступные действия в toolbar:
  ...
  - `Добавить узел` — открывает диалог создания узла
  - `Добавить связь` — активирует режим выбора узлов для создания связи
- Для типов диаграмм:
...
- `src/components/ContextMenu.tsx` — контекстное меню на холсте
- `src/components/AddNodeDialog.tsx` — диалог создания узла (label + shape)
- `src/components/AddEdgeDialog.tsx` — диалог создания связи (метка)
- `src/components/NodeEditor.tsx` — редактор узла (двойной клик)
- `src/components/EdgeEditor.tsx` — редактор связи (двойной клик)
- `src/components/Toolbar.tsx` — панель инструментов (добавлены кнопки)
- `src/components/DiagramCanvas.tsx` — холст с callback'ами (`onCanvasContextMenu`, `onNodeDoubleClick`, `onEdgeDoubleClick`)
